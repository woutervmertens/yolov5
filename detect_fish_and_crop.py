...
def run(weights,  # model.pt path(s)
    ...
    # Directories
    save_dir = increment_path(Path(dest) / name, exist_ok=exist_ok)  # increment run
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_crop:  # Add bbox to image
                        ...
                        if save_crop:
                            save_one_box(xyxy, imc, file=save_dir / 'crops' / names[c] / f'{p.stem}_{conf:.2f}.jpg', BGR=True)
